...
#
# Concepts guarded:
#   1. gid shape regex  ^[0-9a-v]{5,1024}$  — MUST be identical everywhere; 3x
#      (Event ID Valid?, Cancel Lookup ask structOk, Validate Cancel Target).
#   2. confirm_turn canonical regex  ^[1-9][0-9]*$  — 2x (Confirm Fresh?, Verify Confirm Live).
#   3. cancel-target structural rule parity — Cancel Lookup (ask structOk) AND Validate Cancel Target must
#      BOTH validate {finite start_utc, gid shape, calendar_id present}. If either drops one -> drift.
#
...
GID_NODES = ['Event ID Valid?', 'Cancel Lookup', 'Validate Cancel Target']
...
elif gid_forms[('5', '1024')] != 3:
    fails.append(f"gid shape regex count {gid_forms[('5', '1024')]} != 3 — expected in {GID_NODES}, found in {sorted(set(gid_seen))}")
...
for name in ['Cancel Lookup', 'Validate Cancel Target']:
    t = text(name)
    missing = []
...
print('cancel-validation parity OK — gid regex 3x identical [0-9a-v]{5,1024}; confirm_turn regex 2x '
      '^[1-9][0-9]*$; both cancel-target validators check {finite start_utc, gid shape, calendar_id present}')
sys.exit(0)
