...
# Then it git-greps all TRACKED + UNTRACKED (not-yet-added) files for it. Any hit = FAIL.
# `--untracked` covers brand-new files a plain `git grep` would skip (a secret typed into an
# unstaged new file must not sail through the gate — caught by the security-auditor, CP5). It still
# honours .gitignore, so the gitignored CLAUDE.local.md's own N8N_HOST line never false-positives.
#
...
if git grep --untracked -nF -- "$HOST" >/dev/null 2>&1; then
  echo "HOST LEAK — the production host appears in a tracked or new (public) file:"
  git grep --untracked -nF -- "$HOST"
  exit 1
fi

echo "host-leak guard: clean — the production host is not present in any tracked or new file"
exit 0
