# REPO-MAP — tracked files & Markdown `##` anchors

> **Generated by `scripts/repo-map.sh` — do not edit by hand.**
> Regenerate whenever tracked files or headings change (UTC date: 2026-09-14).
>
> The shared map of paths + heading anchors. Cowork and Claude Code both consult
> it before giving/following path-level instructions (see the governance-sync rule
> in `.claude/rules/`). An instruction naming a path/anchor absent here → stop & report.

## Tracked files by directory

### (root)

- `.env.example`
- `.gitignore`
- `.vercelignore`
- `CLAUDE.md`
- `LICENSE`
- `MASTER-BRIEF.md`
- `README.md`

### .claude

- `.claude/settings.json`

### .claude/agents

- `.claude/agents/code-reviewer.md`
- `.claude/agents/flow-reviewer.md`
- `.claude/agents/qa-tester.md`
- `.claude/agents/security-auditor.md`

### .claude/commands

- `.claude/commands/case-study.md`
- `.claude/commands/debug.md`
- `.claude/commands/plan-flow.md`
- `.claude/commands/review.md`
- `.claude/commands/sanitize.md`

### .claude/rules

- `.claude/rules/booking-integrity.md`
- `.claude/rules/code-style.md`
- `.claude/rules/contract-integrity.md`
- `.claude/rules/git-github.md`
- `.claude/rules/governance-sync.md`
- `.claude/rules/handoff.md`
- `.claude/rules/honesty-demos.md`
- `.claude/rules/irreversible-actions.md`
- `.claude/rules/n8n-conventions.md`
- `.claude/rules/plan-gate.md`
- `.claude/rules/prompt-injection.md`
- `.claude/rules/remote-operator.md`
- `.claude/rules/reporting.md`
- `.claude/rules/security-secrets.md`
- `.claude/rules/spend-safety.md`
- `.claude/rules/testing.md`

### config

- `config/client.config.example-b.json`
- `config/client.config.example.json`

### design

- `design/DESIGN-DIRECTIONS.md`
- `design/flow-diagram.md`

### design/mockups

- `design/mockups/.gitkeep`
- `design/mockups/landing.html`
- `design/mockups/tokens.css`

### design/mockups/dashboard

- `design/mockups/dashboard/index.html`
- `design/mockups/dashboard/states.html`

### design/mockups/site

- `design/mockups/site/index.html`

### design/mockups/snippet

- `design/mockups/snippet/barber-widget.js`
- `design/mockups/snippet/host-demo.html`
- `design/mockups/snippet/index.html`

### design/mockups/variants

- `design/mockups/variants/dashboard-a-cream-ink.html`
- `design/mockups/variants/landing-a-cream-ink.html`
- `design/mockups/variants/widget-a-cream-ink.html`

### design/mockups/widget

- `design/mockups/widget/booking.html`
- `design/mockups/widget/cancel-reschedule.html`
- `design/mockups/widget/faq-lead.html`
- `design/mockups/widget/handoff-errors.html`
- `design/mockups/widget/index.html`

### docs

- `docs/ARCHITECTURE-DECISIONS.md`
- `docs/CP4-RESCHEDULE-EXECUTE-PLAN.md`
- `docs/CP6D-BUILD-PACKAGE.md`
- `docs/DATA-MODEL.md`
- `docs/FLOW-DIAGRAM.md`
- `docs/OPERATIONAL-INCIDENTS.md`
- `docs/PHASE-6-BACKLOG.md`
- `docs/REPO-MAP.md`
- `docs/ROADMAP.md`
- `docs/SCREEN-INVENTORY.md`
- `docs/UX-ARCHITECTURE.md`
- `docs/n8n-rollback.md`

### n8n

- `n8n/notes.md`
- `n8n/workflow.drill-state.sanitized.json`
- `n8n/workflow.owner-actions.sanitized.json`
- `n8n/workflow.purge.sanitized.json`
- `n8n/workflow.reminders.sanitized.json`
- `n8n/workflow.sanitized.json`

### n8n/screenshots

- `n8n/screenshots/.gitkeep`

### prompts

- `prompts/intent-extraction.md`

### schemas

- `schemas/client.config.schema.json`
- `schemas/intent.schema.json`

### scripts

- `scripts/check-cancel-validation-parity.py`
- `scripts/check-client-imports.cjs`
- `scripts/check-computed-reply-coverage.py`
- `scripts/check-config-schema.cjs`
- `scripts/check-content-parity.py`
- `scripts/check-deploy-separation.sh`
- `scripts/check-exports-parity.sh`
- `scripts/check-font-provenance.sh`
- `scripts/check-hero-engine-text.cjs`
- `scripts/check-hook-drift.sh`
- `scripts/check-listeners.sh`
- `scripts/check-live-parity.py`
- `scripts/check-no-host-leak.sh`
- `scripts/check-outbound-inventory.py`
- `scripts/compile-intent-validator.cjs`
- `scripts/env-presence.sh`
- `scripts/generate-config-types.cjs`
- `scripts/package-lock.json`
- `scripts/package.json`
- `scripts/repo-map.sh`
- `scripts/secret-scan.sh`

### tests

- `tests/golden-set.md`
- `tests/jailbreak-cases.md`
- `tests/regression-suite.md`
- `tests/run-d11.sh`
- `tests/run-regression.sh`

### tests/snippet

- `tests/snippet/DRILLS.md`
- `tests/snippet/host-demo.html`

### tests/unit

- `tests/unit/locked-once-parity.test.cjs`
- `tests/unit/resolve-date.test.cjs`
- `tests/unit/validate-intent.test.cjs`

### web

- `web/README.md`
- `web/package-lock.json`
- `web/package.json`

### web/dashboard

- `web/dashboard/.gitkeep`
- `web/dashboard/next-env.d.ts`
- `web/dashboard/package.json`
- `web/dashboard/tsconfig.json`

### web/dashboard/app

- `web/dashboard/app/error.tsx`
- `web/dashboard/app/globals.css`
- `web/dashboard/app/layout.tsx`
- `web/dashboard/app/page.tsx`

### web/dashboard/app/api/release

- `web/dashboard/app/api/release/route.ts`

### web/dashboard/components

- `web/dashboard/components/AppointmentsPanel.tsx`
- `web/dashboard/components/HandoffQueue.tsx`
- `web/dashboard/components/LeadsPanel.tsx`
- `web/dashboard/components/ReleaseButton.tsx`
- `web/dashboard/components/States.tsx`
- `web/dashboard/components/SystemHealth.tsx`

### web/dashboard/lib

- `web/dashboard/lib/airtable.ts`
- `web/dashboard/lib/alertState.test.ts`
- `web/dashboard/lib/alertState.ts`
- `web/dashboard/lib/mask.test.ts`
- `web/dashboard/lib/mask.ts`
- `web/dashboard/lib/ownerAction.test.ts`
- `web/dashboard/lib/ownerAction.ts`
- `web/dashboard/lib/shopConfig.ts`
- `web/dashboard/lib/types.ts`

### web/shared

- `web/shared/package.json`

### web/shared/src/chat

- `web/shared/src/chat/chatClient.test.ts`
- `web/shared/src/chat/chatClient.ts`

### web/shared/src/config

- `web/shared/src/config/client.config.types.ts`
- `web/shared/src/config/loadConfig.test.ts`
- `web/shared/src/config/loadConfig.ts`

### web/site

- `web/site/next-env.d.ts`
- `web/site/next.config.mjs`
- `web/site/package.json`
- `web/site/tsconfig.json`

### web/site/app

- `web/site/app/globals.css`
- `web/site/app/layout.tsx`
- `web/site/app/page.tsx`

### web/site/app/install

- `web/site/app/install/page.tsx`

### web/site/components/site

- `web/site/components/site/Closer.tsx`
- `web/site/components/site/Craft.tsx`
- `web/site/components/site/Hero.tsx`
- `web/site/components/site/HeroChat.tsx`
- `web/site/components/site/HoursPlace.tsx`
- `web/site/components/site/LiveChatPanel.tsx`
- `web/site/components/site/Menu.tsx`
- `web/site/components/site/MockRibbon.tsx`
- `web/site/components/site/SiteChrome.tsx`
- `web/site/components/site/SiteFooter.tsx`
- `web/site/components/site/SiteHeader.tsx`
- `web/site/components/site/SiteMotion.tsx`
- `web/site/components/site/SitePanel.tsx`
- `web/site/components/site/TurnstileWidget.tsx`
- `web/site/components/site/motion-engine.js`

### web/site/lib

- `web/site/lib/config.ts`
- `web/site/lib/endpoint.ts`
- `web/site/lib/heroScenarios.ts`
- `web/site/lib/lockedOnce.ts`

### web/site/public/fonts

- `web/site/public/fonts/OFL.txt`
- `web/site/public/fonts/PROVENANCE.md`
- `web/site/public/fonts/fraunces-variable.woff2`
- `web/site/public/fonts/instrument-sans-variable.woff2`

### web/site/scripts

- `web/site/scripts/build-config.mjs`

### web/snippet

- `web/snippet/CROSS-ORIGIN.md`
- `web/snippet/build.mjs`
- `web/snippet/package.json`
- `web/snippet/tsconfig.json`

### web/snippet/src

- `web/snippet/src/fonts.ts`
- `web/snippet/src/index.ts`
- `web/snippet/src/lockedOnce.ts`
- `web/snippet/src/styles.ts`
- `web/snippet/src/turnstile.ts`

## Markdown files — their `##`/`###` headings (anchors; code fences skipped)

### `.claude/agents/code-reviewer.md`
  - _(no `##` headings — flat file)_

### `.claude/agents/flow-reviewer.md`
  - _(no `##` headings — flat file)_

### `.claude/agents/qa-tester.md`
  - _(no `##` headings — flat file)_

### `.claude/agents/security-auditor.md`
  - _(no `##` headings — flat file)_

### `.claude/commands/case-study.md`
  - _(no `##` headings — flat file)_

### `.claude/commands/debug.md`
  - _(no `##` headings — flat file)_

### `.claude/commands/plan-flow.md`
  - _(no `##` headings — flat file)_

### `.claude/commands/review.md`
  - _(no `##` headings — flat file)_

### `.claude/commands/sanitize.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/booking-integrity.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/code-style.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/contract-integrity.md`
  - ## The rule
  - ## Why
  - ## Derived copies

### `.claude/rules/git-github.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/governance-sync.md`
  - ## Why
  - ## The rules

### `.claude/rules/handoff.md`
  - ## Infrastructure failure ≠ conversational handoff

### `.claude/rules/honesty-demos.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/irreversible-actions.md`
  - ## Why the prompt was removed rather than kept
  - ## The rule
  - ## Why this list and not a longer one
  - ## Related

### `.claude/rules/n8n-conventions.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/plan-gate.md`
  - ## The gate
  - ## Scope note
  - ## Why

### `.claude/rules/prompt-injection.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/remote-operator.md`
  - ## The rule
  - ### The rule held for the NEXT bind and did nothing about the ones already up — so there is a sweep now
  - ### The policy — **root output, Yigitcan ran it 2026-09-12. NOT my measurement.**
  - ## ⛔ SSH NEVER GOES TO A CLOUDFLARE-PROXIED HOSTNAME — and here is WHY, so it is not re-derived
  - ### NEVER TYPE THE LITERAL — not in a file, not in a command you SHOW
  - ### Pre-flight before handing over ANY tunnel command
  - ## Why
  - ## Related: do not send him a drill you can run yourself
  - ## The capability FACT: this machine HAS a browser, and it is Claude's to drive

### `.claude/rules/reporting.md`
  - ## ONE VERDICT per message, and it is the LAST thing in it
  - ### A relayed SUB-AGENT verdict is evidence, not the turn's verdict
  - ## Live-state sync is part of closing a phase (no drift)
  - ## A claim ABOUT a guard is written AFTER the mutation test, never beside the code
  - ## An EMPTY result is a claim about the INSTRUMENT until the instrument has been checked
  - ## A STRUCTURAL claim needs evidence too, not just a behavioural one
  - ## Evidence before abandoning a planned approach
  - ## "The operator did it" is not a measurement — and the measuring is YOURS
  - ## A claim about your OWN CAPABILITY is a claim like any other

### `.claude/rules/security-secrets.md`
  - ## What the deny glob actually does — MEASURED, not assumed
  - ### ⚠ The deny layer is ANTI-ACCIDENT. It is NOT a boundary against a determined reader.
  - ### Reading a file out of git history — the bound

### `.claude/rules/spend-safety.md`
  - _(no `##` headings — flat file)_

### `.claude/rules/testing.md`
  - _(no `##` headings — flat file)_

### `CLAUDE.md`
  - ## What this is
  - ## Working language
  - ## Operating protocol (non-negotiable)
  - ## Defense-in-depth (5 layers)
  - ## Security — repo is PUBLIC (critical)
  - ## Definition of Done (per phase)
  - ## Rules & commands

### `MASTER-BRIEF.md`
  - ## 1. Operating protocol for Claude Code (READ FIRST)
  - ## 2. Division of labor
  - ## 3. Project identity
  - ## 4. Pre-build brief
  - ## 5. Architecture — components & where each runs
  - ## 6. Config / Code / Secret separation (the template mechanism)
  - ## 7. Mandatory core controls (T1 chatbot)
  - ## 8. Data flow
  - ## 9. Critical-Review Targets (Codex audit — gate before "done")
  - ## 10. Roadmap (Phase 0 → 8) + Definition of Done
  - ## 11. Where `/repo-scaffold`, `/loop`, and `/goal` are used
  - ## 12. Decision log (deviations & rationale)
  - ## 13. Assumptions to verify (⚠ — NOT yet confirmed; do not treat as fact)

### `README.md`
  - ## The one job
  - ## How it works
  - ## What works today ✅
  - ## Designed, locked, not built yet 🔜
  - ## Engineering decisions worth reading
  - ## Case study — the embeddable widget (Phase 6b)
  - ## Repo map
  - ## Security
  - ## From demo to real
  - ## Documentation

### `design/DESIGN-DIRECTIONS.md`
  - ## 0. Araştırma Özeti (kaynaklar en altta)
  - ### İncelenen ödüllü referanslar (canlı gezildi, screenshot'landı)
  - ### 2026 motion/tip trendleri (Figma, TheeDigital, Elementor trend raporları)
  - ### Booking conversion UX (Booksy, Trafft, Baymard verileri)
  - ## 1. Ortak Temel (üç yönde de geçerli — pazarlık yok)
  - ## 2. Direction A — «Crema & Mürekkep» *(marka-odaklı, editorial craft)*
  - ## 3. Direction B — «Signal» *(ifade-odaklı, bold pop)*
  - ## 4. Direction C — «Atelier OS» *(ürün-odaklı, quiet luxury × SaaS)*
  - ## 5. Mockup Fazı Planı
  - ## Kaynaklar

### `design/flow-diagram.md`
  - _(no `##` headings — flat file)_

### `docs/ARCHITECTURE-DECISIONS.md`
  - ## 1. Snapshot
  - ## 2. The plan (locked)
  - ## 3. Stack
  - ## 4. Non-negotiables applied
  - ## 5. Decision log
  - ## 6. Final Feature Log (locked 2026-07-04)
  - ### Booking lifecycle
  - ### Security (director additions)
  - ### Bot capabilities
  - ### Handoff
  - ### WhatsApp transport (Zernio — VERIFIED, official WhatsApp Business API BSP)
  - ### Frontend
  - ### Technical decisions (schemas updated in Phase 2/3 — DECISION ONLY now)
  - ### Future (post-v1, NOT a phase)
  - ### Conscious v1 boundary (stated honestly in the Phase 7 README/case-study)
  - ## 7. Critical-Review Targets — additions (2026-07-04)

### `docs/CP4-RESCHEDULE-EXECUTE-PLAN.md`
  - ## What this replaces
  - ## Inputs available at `Reschedule Fresh? [out0 fresh]`
  - ## Locked order (MANDATORY — never reorder)
  - ### (a) Open the new booking
  - ### (b) Verify the new booking (write-then-verify, no double-book on the NEW slot)
  - ### (c) Commit the move
  - ## Failure builders (all set `computed_reply`; all → `Save State (Post-Write)` except NeedsHuman → `Save State`)
  - ## Config templates to add (both places: `config/client.config.example.json` AND the `Load Config` node)
  - ## Decisions (record in ARCHITECTURE-DECISIONS.md §5 at commit)
  - ## ⚙ Drill matrix (ALL hand-run BEFORE commit; execution API + Airtable column, NEVER the reply)
  - ## Build method
  - ## Incremental build order (one verifiable step at a time — NEVER 13 nodes at once)
  - ## Stop conditions (unchanged)

### `docs/CP6D-BUILD-PACKAGE.md`
  - ## STEP 0 — the shared secret (Yigitcan; the ONE thing CC does not do)
  - ## PART A — the D11 owner write path (11 nodes, new workflow *or* a new trigger in the main one)
  - ### A1 · `Receive Owner Action` — Webhook
  - ### A2 · `Verify Owner Signature` — Code
  - ### A3 · `Owner Request Fresh?` — Code + IF  ⟵ replay window
  - ### A4 · `Signature Valid?` — IF
  - ### A5 · `Validate Owner Action` — Code  ⟵ **the allow-list**
  - ### A6 · `Find Target Conversation` — Airtable (get by record id)
  - ### A7 · `Validate Release Target` — Code  ⟵ **target validation**
  - ### A8 · `Check Owner Replay` — Airtable (search) on `processed_messages` by `messageId`
  - ### A9 · `Release Handoff Lock` — Airtable (update)
  - ### A10 · `Re-read Released Conversation` — Airtable (get)  ⟵ **write-then-verify**
  - ### A11 · `Build Owner Alert (Release)` — Code → existing `Send Owner Alert (Telegram)`
  - ### A12 · `Respond Owner OK` — Respond to Webhook — **200**
  - ## PART B — the `leads` TTL branch (a NEW branch in the EXISTING purge workflow)
  - ### B1 · `Compute Leads Cutoff` — Code (from `Load Config (Purge)`)
  - ### B2 · `Find Old Leads` — Airtable (search)
  - ### B3 · `Leads Purge Sane?` — IF  ⟵ **THE CANDIDATE CAP**
  - ### B4 · `Count Leads Before` — Airtable (search, count only)
  - ### B5 · `Delete Old Leads` — Airtable (deleteRecord), **batched at 25 per request**
  - ### B6 · `Count Leads After` — Airtable (search, count only)
  - ### B7 · `Verify Leads Purge` — Code: assert `before − after === deleted`; mismatch → alert branch.
  - ### B8 · errors → the existing `Build Owner Alert (Purge)`
  - ## After building

### `docs/DATA-MODEL.md`
  - ## `leads`
  - ## `appointments`
  - ## `bot_metrics` (CP5b spend-cap rolling counter — NOT PII)
  - ## `conversations` (multi-turn slot-filling state)
  - ## `processed_messages` (idempotency / dedupe)

### `docs/FLOW-DIAGRAM.md`
  - ## Overview — the lanes
  - ## Lane 1 — Ingress · idempotency · state load
  - ## Lane 2 — Guards (before the LLM) · LLM intent · validate · gate
  - ## Lane 2b — Perimeter brakes (CP5b): rate-limit · Turnstile · spend-cap · dry-run
  - ## Lane 3 — Route by intent
  - ## Lane 4 — Booking: slot-filling → availability (read-only)
  - ## Lane 5 — Confirm → write-then-verify → reconcile (no-double-book)
  - ## Lane 6 — Cancel: fail-closed confirm lifecycle
  - ## Lane 8 — Reschedule: discovery + fail-closed confirm (CP4)
  - ## Lane 8b — Reschedule EXECUTE: book-new-first → verify/race → commit
  - ## Reply lane — one exit for every conversational reply
  - ## Owner-alert lane — every failure/handoff reaches a human (CP5a)
  - ## The invariants this diagram encodes (why the shapes are what they are)

### `docs/OPERATIONAL-INCIDENTS.md`
  - ## OPS-1 · 2026-09-09 · A resync step sanitized the LIVE workflow — production broke, and the parity guard stayed GREEN
  - ## OPS-2 · 2026-09-11 · `pkill -f` matched its own shell and killed it mid-script; a server survived 2h37m
  - ## OPS-3 · 2026-09-11 · `git clone --local` failed silently, so a subagent's negative control ran in the REAL repo
  - ## OPS-4 · 2026-09-11 · The auditor could not read the body of the guard it was judging

### `docs/PHASE-6-BACKLOG.md`
  - ## 1. Empty / loading / error states — *dashboard, widget*
  - ## 2. Dashboard interactions (no design yet) — *dashboard*
  - ## 3. Mobile — *all three surfaces (verified 2026-07-04 at 390px)*
  - ## 4. Landing scope — DECISION NEEDED — *landing*
  - ## 5. Conversation spec — *Phase 2 core (not Phase 6)*
  - ## 6. Minor
  - ## 7. Dashboard authentication — SECURITY-CRITICAL — *dashboard*
  - ## 8. Dark/light theme — *all three surfaces*

### `docs/REPO-MAP.md`
  - ## Tracked files by directory
  - ### (root)
  - ### .claude
  - ### .claude/agents
  - ### .claude/commands
  - ### .claude/rules
  - ### config
  - ### design
  - ### design/mockups
  - ### design/mockups/dashboard
  - ### design/mockups/site
  - ### design/mockups/snippet
  - ### design/mockups/variants
  - ### design/mockups/widget
  - ### docs
  - ### n8n
  - ### n8n/screenshots
  - ### prompts
  - ### schemas
  - ### scripts
  - ### tests
  - ### tests/snippet
  - ### tests/unit
  - ### web
  - ### web/dashboard
  - ### web/dashboard/app
  - ### web/dashboard/app/api/release
  - ### web/dashboard/components
  - ### web/dashboard/lib
  - ### web/shared
  - ### web/shared/src/chat
  - ### web/shared/src/config
  - ### web/site
  - ### web/site/app
  - ### web/site/app/install
  - ### web/site/components/site
  - ### web/site/lib
  - ### web/site/public/fonts
  - ### web/site/scripts
  - ### web/snippet
  - ### web/snippet/src
  - ## Markdown files — their `##`/`###` headings (anchors; code fences skipped)
  - ### `.claude/agents/code-reviewer.md`
  - ### `.claude/agents/flow-reviewer.md`
  - ### `.claude/agents/qa-tester.md`
  - ### `.claude/agents/security-auditor.md`
  - ### `.claude/commands/case-study.md`
  - ### `.claude/commands/debug.md`
  - ### `.claude/commands/plan-flow.md`
  - ### `.claude/commands/review.md`
  - ### `.claude/commands/sanitize.md`
  - ### `.claude/rules/booking-integrity.md`
  - ### `.claude/rules/code-style.md`
  - ### `.claude/rules/contract-integrity.md`
  - ### `.claude/rules/git-github.md`
  - ### `.claude/rules/governance-sync.md`
  - ### `.claude/rules/handoff.md`
  - ### `.claude/rules/honesty-demos.md`
  - ### `.claude/rules/irreversible-actions.md`
  - ### `.claude/rules/n8n-conventions.md`
  - ### `.claude/rules/plan-gate.md`
  - ### `.claude/rules/prompt-injection.md`
  - ### `.claude/rules/remote-operator.md`
  - ### `.claude/rules/reporting.md`
  - ### `.claude/rules/security-secrets.md`
  - ### `.claude/rules/spend-safety.md`
  - ### `.claude/rules/testing.md`
  - ### `CLAUDE.md`
  - ### `MASTER-BRIEF.md`
  - ### `README.md`
  - ### `design/DESIGN-DIRECTIONS.md`
  - ### `design/flow-diagram.md`
  - ### `docs/ARCHITECTURE-DECISIONS.md`
  - ### `docs/CP4-RESCHEDULE-EXECUTE-PLAN.md`
  - ### `docs/CP6D-BUILD-PACKAGE.md`
  - ### `docs/DATA-MODEL.md`
  - ### `docs/FLOW-DIAGRAM.md`
  - ### `docs/OPERATIONAL-INCIDENTS.md`
  - ### `docs/PHASE-6-BACKLOG.md`
  - ### `docs/REPO-MAP.md`

### `docs/ROADMAP.md`
  - ### ⚠ Phase 5 sequencing & live-endpoint status (2026-08-24)
  - ## Phase 2 — checkpoint progress (core bot, built in n8n)
  - ## Phase 3 — checkpoint progress (booking + data)
  - ## Phase 4 — checkpoint progress (WhatsApp / Zernio)
  - ## Phase 5 — checkpoint progress (safety)
  - ## FIX-1 — Phase 6 öncesi düzeltme turu (DONE 2026-08-30)
  - ## FIX-2 — Design öncesi bulgu kapatma (DONE 2026-08-31)
  - ## TASARIM AŞAMASI — DONE 2026-09-02 (`c491c36` → `46385ed`)
  - ## Phase 6 — checkpoint progress (vitrin frontend)
  - ### 6b DEFINITION OF DONE — KALEM KALEM (testing.md)
  - ### 6b KAPANIŞINDA DEVREDİLEN AÇIK MADDELER — HER BİRİNİN SAHİBİ YAZILI
  - ## FAZ 6 — KAPANDI ✅ (2026-09-14, Yigitcan'ın hükmü)
  - ## Critical-Review Targets (Codex gate — from MASTER-BRIEF §9)

### `docs/SCREEN-INVENTORY.md`
  - ## §0 — Amaç, kural, görsel dil
  - ### Neden bu belge var
  - ### KURAL (bağlayıcı)
  - ### Tur A yöntem kararı (Yigitcan, 2026-08-31 — grup 1 kabulünde)
  - ### Görsel dil — SIFIRDAN BAŞLAMA, MEVCUDU GENİŞLET
  - ## §1 — YÜZEY 1: BERBER DEMO SİTESİ (public, Vercel)
  - ### Bölümler
  - ### Config-driven olan / olmayan (§6'nın özeti)
  - ### NOT-surface (bu sayfanın YAPMADIĞI)
  - ## §2 — YÜZEY 2: WIDGET (sohbet balonu)
  - ### 2.0 — Bileşen anatomisi (mevcut, korunacak)
  - ### 2.1 — Açılış
  - ### 2.2 — FAQ (7 konu, deterministik; LLM yalnız konuyu sınıflandırır)
  - ### 2.3 — Randevu alma
  - ### 2.4 — Müsaitlik reddi (hepsi **literal**, `messageTemplates`'te karşılığı YOK)
  - ### 2.5 — İptal
  - ### 2.6 — Erteleme
  - ### 2.7 — Booking sonrası belirsizlik (yarış / doğrulama)
  - ### 2.8 — Lead yakalama
  - ### 2.9 — İnsana devir — **ÜÇ SINIF, AYRI EKRAN**
  - ### 2.10 — Hata ve kenar durumları
  - ### 2.10.1 — Metni olmayan çıkışların KARARI (BULGU-6 kapandı, K1 uygulandı)
  - #### 2.10.1a — UYGULAMA DURUMU ve iki YENİ frontend durumu (2026-09-11, `code-reviewer` Tur 3 yeniden koşusu)
  - ### 2.11 — Guard durumlarının görünmeyen yüzü
  - ### 2.12 — Stage makinesi (9 değer — Design'ın durum grafiği)
  - ## §3 — YÜZEY 3: GÖMÜLEBİLİR SNIPPET
  - ## §4 — YÜZEY 4: DASHBOARD (özel, Cloudflare Access arkasında)
  - ### 4.0 — Airtable bütçesi (BAĞLAYICI)
  - ### 4.a — Randevular
  - ### 4.b — Lead'ler
  - ### 4.c — DEVİR KUYRUĞU (dashboard'ın asıl işi)
  - #### ⚠ D9'un en kritik tasarım şartı — üç durum KARIŞTIRILMAMALI
  - #### Alert sınıfları — **24 tane** (Design her birini bir rozet/etiket olarak ele almalı)
  - ### 4.d — Sistem sağlığı
  - ### 4.e — Ortak dashboard durumları
  - ## §5 — ORTAK ÖĞELER (tüm yüzeylerde)
  - ## §6 — CONFIG-DRIVEN vs SABİT (template iddiasının kanıtı)
  - ## §7 — BOŞLUKLAR ve ÇELİŞKİLER
  - ### ⚠ BULGU'lar (sistem/doküman çelişkisi — bu tarama sırasında bulundu)
  - ### Sistemin ürettiği ama ekranı olmayan
  - ### Ekranı gerekip de sistemde karşılığı olmayan
  - ## §8 — AÇIK KARARLAR (Design'a gitmeden önce)
  - ### K1 — Metni olmayan çıkışlar ne gösterecek? ✅ **KARARA BAĞLANDI → §2.10.1**
  - ### K2 — Devir kilidinde (W47) girdi çubuğu ne yapsın? ✅ **KARARA BAĞLANDI → C (Tur A brief'i, 2026-08-31)**
  - ### K3 — Lead durum değiştirme Phase 6'da olsun mu?
  - ### K4 — Yeni ziyaretçi için ayrı karşılama ekranı olsun mu? ✅ **KARARA BAĞLANDI → B (2026-08-31)**
  - ### K5 — Mock şeridi gerçek müşteri kurulumunda nasıl kalkacak? *(BULGU-3 kapandığı için engel kalktı)*
  - ### K6 — Dashboard "son hatalar" paneli (D16) ne olacak?
  - ### K7 — Landing'de yapılandırılmış adres gösterilsin mi? ✅ **UYGULANDI**
  - ## §9 — DESIGN'A TESLİM ÖZETİ

### `docs/UX-ARCHITECTURE.md`
  - ## §0 — Amaç ve türetme kuralı
  - ### Neden bu adım var
  - ### Türetme kuralı (bağlayıcı)
  - ### Bu belge NE DEĞİL
  - ## §1 — Aktörler ve yüzeyler
  - ### NOT-surface — her yüzeyin YAPMADIĞI
  - ### Instagram
  - ### Zernio (WhatsApp BSP) — güncel durum
  - ## §2 — Müşteri akışları
  - ### 2.1 — Karşılama (yeni / dönen müşteri)
  - ### 2.2 — FAQ (7 konu, deterministik — LLM cevabı değil)
  - ### 2.3 — Randevu alma (servis → tarih/saat → müsaitlik → onay → booked)
  - ### 2.4 — Slot dolu → alternatifler
  - ### 2.5 — İptal (bul → onayla → sonuç)
  - ### 2.6 — Erteleme (bul → yeni slot → onayla → taşındı)
  - ### 2.7 — Lead yakalama
  - ### 2.8 — İnsana devir — **3 sınıf, asla birleştirilmez**
  - ### 2.9 — Hatırlatma (giden mesaj, UI yok)
  - ### 2.10 — Guard-trip'ler (müşterinin gördüğü)
  - ## §3 — Müşteri hata / kenar durum ekranları
  - ### 3.1 — Converged çıkışlar (12 dal → `Finalize Outbound` → `Channel Switch`)
  - ### 3.2 — Converged OLMAYAN çıkışlar (front-gate, doğrudan cevap)
  - ### 3.3 — n8n dışındaki kenar durumlar
  - ### 3.4 — Sistem anahtarlarının müşteri yüzü
  - ## §4 — Sahip (dashboard) bilgi mimarisi
  - ### 4.a — Randevular
  - ### 4.b — Lead'ler
  - ### 4.c — DEVİR KUYRUĞU — **en kritik bölüm**
  - ### 4.d — Sistem sağlığı
  - ### 4.e — Config anahtarlarının şema durumu (Phase 6'nın "config-wired" hedefi için)
  - ## §5 — Aksiyon sözleşmesi (mimari şart — pazarlıksız)
  - ### Aksiyon → mevcut yol eşlemesi
  - ## §6 — Deploy topolojisi (karar verildi)
  - ### TAŞINABİLİRLİK KISITI (bağlayıcı)
  - ## §7 — Veri katmanı gerçeği
  - ### Ne yok
  - ### Phase 6 için SOMUT risk: Airtable API rate limit
  - ### Airtable'ın sert limiti (DOĞRULANDI — Airtable resmi dokümanı, sahip teyidi 2026-08-27)
  - ### TASARIM KISITI (bağlayıcı — Phase 6 bunu ihlal edemez)
  - ### DB'ye geçiş TETİKLEYİCİLERİ (karar sürüklenmesin)
  - ## §8 — Boşluklar ve çelişkiler
  - ### 8.1 — ⚠ BULGU: sistem çelişkileri (envanterden çıktı, hepsi doğrulandı)
  - ### 8.2 — Yüzeysiz kalanlar (envanterde var, hiçbir ekranda yok)
  - ### 8.3 — Şema / config boşlukları
  - ## §9 — Kararlar (KAPANDI — Yigitcan, 2026-08-27)
  - ### K1 — Devir kuyruğu kaynağı → **conversations satırına iki alan**
  - ### K2 — Landing kapsamı → **odaklı bot vitrini**
  - ### K3 — Dashboard veri erişimi → **kendi API katmanı**
  - ### K4 — Widget 400/503 metni → **config template (mevcut anahtarlar, YENİ anahtar YOK)**
  - ### K5 — `outsideHours` + `greeting` → **tasarımdan çıkar + ölü config anahtarlarını TEMİZLE**
  - ### K6 — Phase 6 kapsamı → **salt-okunur + devir kuyruğu, TEK yazma istisnası ile**
  - ### K7 — BULGU #1 zamanlaması → **Phase 6'dan ÖNCE, ayrı küçük fix turu**
  - ## §10 — Kararların uygulama dağılımı
  - ### §10.1 — Kabul edilen ama ADLANDIRILMIŞ kalıntı: hatırlatma penceresi
  - ## Kapanış — bu belge neyi garanti eder

### `docs/n8n-rollback.md`
  - ## ⚠ Do NOT trust `n8n_workflow_versions` for THIS workflow
  - ## Snapshot — before EACH refactor step (one per step, never reused)
  - ## Rollback — if a step breaks the suite
  - ## ⚠ `b1` (Build Reply Payload) has a mandatory extra gate
  - ## Close gate — run AFTER publish + suite, BEFORE commit (every refactor step)
  - ## Standing rules (Ö3–Ö5) during refactor
  - ## Rehearsal log
  - ## ⚠ API save blocked by `settings must NOT have additional properties` (2026-08-18)

### `n8n/notes.md`
  - ## Files
  - ## Rules that apply
  - ## Runtime (from MASTER-BRIEF §5)

### `prompts/intent-extraction.md`
  - ## System prompt (v4 — 2026-09-09, fail-closed date contract)
  - ## Few-shot examples
  - ## Notes

### `tests/golden-set.md`
  - _(no `##` headings — flat file)_

### `tests/jailbreak-cases.md`
  - _(no `##` headings — flat file)_

### `tests/regression-suite.md`
  - ## Scenario table
  - ### Reschedule (CP4 sub-step 3 — insert-new + verify/race + commit; book-new-first). "R:" prefixes the reschedule execute nodes.
  - ### Pre-hours class fix — a REJECTED availability slot must not remain bookable
  - ### Phase 4 — Zernio inbound adapter + signature (CP4a) — ⚙ assisted (nested payload + HMAC signing; not in the widget-only `run-regression.sh`)
  - ### Phase 4 — outbound lane (CP4b-1 convergence · CP4b-2 whatsapp send · CP4b-3 5xx/ACK policy)
  - ### Phase 4 — CP4c reminder Zernio TEMPLATE send (STUB → real, dry-run gated) — ⚙ assisted
  - ### Phase 4 — CP4d-1 real WhatsApp e2e (shared Zernio sandbox, LIVE — 2026-08-23)
  - ### ⚙ Owner-alert drills (CP5a) — execution API + real Telegram delivery, NEVER the reply text
  - ## Baseline run
  - ### Known-failing under a full rapid run — flagged for Phase 7 CRT #1a review (NOT dismissed as flake)
  - ### Exit & branch coverage map (Ö2 — every reply exit + branch accounted for)
  - ### ⚙ Turnstile gate — TWO-WAY probe (added 2026-09-04 after the gate was found open)
  - ### ⚙ T2e — the widget's self-diagnosis probe (NOT YET DRILLED, added 2026-09-06)
  - ### ⚙ CP5b perimeter-brake drills (spend-cap + dry-run) — exec-API + Airtable column, self-cleaning
  - ### ⚙ CP5d signature/adapter drills (constant-time compare + Normalize graceful reject)
  - ### ⚙ CP5e robustness drills (Airtable error-split · purge · reminders overlap lock)
  - ### ⚙ Reconcile drills — Phase-7 gate (from refactor Step 1 / c1)
  - ### Clarify tier — `unknown` ≠ `handoff` (added 2026-09-07, all Airtable-verified)
  - ### Date resolution — arithmetic taken away from the LLM (added 2026-09-07, CRT #12)

### `tests/snippet/DRILLS.md`
  - ## Who runs what — the dividing question is whether a REAL TURNSTILE TOKEN is needed
  - ### Run headless by Claude — RESULTS (2026-09-10)
  - ### Drilling `scripts/secret-scan.sh` — the false green that catches everyone
  - ### Needs Yigitcan's browser — RESULT (2026-09-11, re-run on the CURRENT bundle)
  - ### Needs Yigitcan's browser — the ONLY one
  - ## The rule this sheet exists to enforce

### `web/README.md`
  - ## Deploy topology (decided — UX-ARCHITECTURE §6, Phase 6 decision D-1)
  - ### Binding constraints
  - ## Config-driven
  - ## Browser-visible build inputs (slice 2 — the live widget)
  - ## Commands
  - ### Fresh-clone order: BUILD, then CHECK (not the other way round)
  - ### The build-time server/client boundary
  - ## Source of truth for what gets built

### `web/site/public/fonts/PROVENANCE.md`
  - ## Fraunces
  - ## Instrument Sans
  - ## Re-verify
  - ## Why self-hosted at all
  - ## Open cost question (raised, not decided)

### `web/snippet/CROSS-ORIGIN.md`
  - ## Reading the measurement
  - ## Not present today (checked, so the absence is a measurement rather than an assumption)
