...
- ✅ **CP2 — Booking write (DONE):** split 2a-i/2a-ii/2b/2c — all seven sub-items closed below. ⚠ *This line read `▶ IN PROGRESS` until 2026-09-13, inside a phase the top table has shown as ✅ for weeks: a parent marker that was never updated when its last child closed. Derived from the children, not from memory.*
...
- ✅ **CP4 — Reschedule (DONE):** all eight sub-items closed below. ⚠ *Same stale-parent defect as CP2 above, corrected 2026-09-13.* Design = insert-new + delete-old, book-new-first, own `Reschedule Fresh?` TTL gate (CRT #8 MEDIUM requirement), 4 sub-steps. Decision A (ARCH-DEC §5): reschedule uses the booking availability rules, **no min-lead-time** (→ Phase 8 backlog).
...
- ✅ **CP4b — Outbound (Zernio send + 5xx policy) (DONE):** all three sub-items closed, and the repo states it directly — *"CP4b outbound/5xx + CP4c reminder-template + CP4d-1 real sandbox e2e DONE"*. ⚠ *Parent marker corrected 2026-09-13; it had read `▶` while every child was ✅.* converge the 11 terminal respond nodes to a channel-aware outbound.
...
- ☐ **6c — dashboard, read-only + handoff queue.** Own server behind Cloudflare Access; own API layer; one bulk read per page.
  - ✅ **CP 6c-0 — the precondition gate. Plan APPROVED 2026-09-11 · CLOSED 2026-09-12 (`e9c2cac`, pushed)
    ...
    - ✅ **CP 6c-1 — the dashboard API layer (CRT #10). CLOSED 2026-09-12 (`b097053`, pushed).**
      ...
      - ✅ **CP 6c-3 — the closing round. CLOSED 2026-09-13 (`c324522`, pushed), with a follow-up ledger
        commit (`58fcf2b`).** Six items, the budget's last round. Every acceptance below is a MEASUREMENT; none closes on "it is in the DOM".
...
**→ PUBLIC-DEPLOY KAPISI — TEK VE EKSİKSİZ LİSTE (2026-09-14).** Bugüne dek bu maddeler üç ayrı yere
dağılmıştı ve tarama iki kusur buldu: **17 numara İKİ satırda birden** kullanılmıştı, ve **Privacy Addendum
zaten 14'tü** — 2026-09-13'te 6b'den "taşıdığım" satır, var olan bir kapı maddesinin KOPYASIydı. Numaralar
burada bir kez ve sırayla veriliyor; kopya birleştirildi. Vitrin bunların hepsi kapanmadan herkese açılmaz.
...
| 13 | **`leads` TTL/purge — DAL İNŞA EDİLDİ ve drill edildi (CP 6d-1), ama CANLI SİLME hiç koşmadı.** Aday kapı, aritmetik doğrulama ve fail-closed davranışı drill satırlarıyla iki yönde kanıtlandı; gerçek bir retention silmesi gerçek verinin üstünde **hiç gerçekleşmedi**. Kapı, inşa değil CANLI DOĞRULAMA. | yabancı biri public demo'ya gerçek telefon yazabiliyor (D-11); TTL'in çalıştığı gösterilmeden açılmaz |
| 14 | **Privacy Addendum — Cloudflare'in Turnstile kullanan bir siteye bağladığı yükümlülük. YARISI kapalı.** *(2026-09-13'te 6b'den buraya taşındığı sanılan satır aslında bunun kopyasıydı; 2026-09-14'te birleştirildi.)* | yayın öncesi hukuki metin + atıf |
...
| 17 | **CF_API_TOKEN İPTAL / Read'e düşürme + BU TURDA SIZAN DÖRT CREDENTIAL'IN ROTASYONU.** Token'ın kapsamı 6d için genişletilmişti ve iş bitti. ⚠ Ayrıca 2026-09-13'te CC'nin bir maskeleme hatası dört gerçek değeri RAPORA bastı (iki CF service token · n8n API key · Telegram chat id); repo görmedi, rapor gördü. Faz 7 build'inden ÖNCE, tek oturumda. | ayrıcalıklı kimlik, işi bitmiş; ve sızmış kimlik rotasyonsuz kapanmaz |
...
| 19 | **SAHİBİN PANOYA İNTERNETTEN ERİŞEBİLDİĞİ HİÇ GÖSTERİLMEDİ.** D11 yolu tünelden uçtan uca kanıtlandı — düğme tıklandı, Airtable değişti — ama pano yalnız `127.0.0.1`'e bağlanıp SSH tüneliyle sürüldü. "Sahibi internetten açıp basabiliyor" ayrı bir olgudur ve ÖLÇÜLMEDİ. | konuşlanma olgusu; ürün iddiası bunun üstünde duruyor |
| 20 | **Loom walkthrough** — K-3 hükmüyle (2026-09-13) sevk kapısına taşındı ve bugüne dek yalnız `CLAUDE.md` ile `.claude/rules/testing.md` düzyazısında yaşıyordu, numaralı listede değil. | Yigitcan'ın sistemi ANLATABİLMESİ, el ile tıklamayla değil artefaktla korunuyor |
...
## FAZ 6 — KAPANDI ✅ (2026-09-14, Yigitcan'ın hükmü)

**Dört satır, aynen ve değiştirilmeden — dürüstlük burada yaşıyor, işarette değil:**

- ✅ **release path built and proven through the tunnel**
- ❌ **reachable by the owner from the internet** → sevk kapısı (madde 19)
- ✅ **Access applies Service Auth correctly on this account**
- ❌ **Access protects the dashboard** → sevk kapısı (madde 18)

**HÜKMÜN GEREKÇESİ, çünkü tik'in kendisi bir iddiadır ve gerekçesi olmadan okunamaz.** Faz 6'nın
teslimlerinin hepsi — 6a demo sitesi ve widget, 6b gömülebilir snippet, 6c pano, 6d sahibin tek yazma
yolu ve `leads` TTL dalı — **inşa edildi ve drill edildi.** Kalan iki ❌ inşa eksiği değil, **KONUŞLANMA
OLGUSUDUR**, ve konuşlanma bu fazda bilinçli olarak kapsam dışıydı: Vercel projesi yok (2026-09-12'de
ölçüldü), pano yalnız loopback'e bağlanıyor. **Hiç kapsamda olmamış bir şey için bir fazı süresiz `⚠`
tutmak, işaretin anlamını yok eder** — o zaman `⚠` "yapılmadı" değil "bir gün başka bir şey de olacak"
demeye başlar, ve hiçbir faz bir daha kapanmaz.

**Bunun overclaim OLMAMASINI sağlayan şey:** ✅ yalnız FAZIN teslimlerini kapsıyor, ürünün sevk
edilebilirliğini değil. Sevk edilebilirlik ayrı ve numaralı bir kapıdır (yukarıdaki 11–20), vitrin o
liste kapanmadan herkese açılmaz, ve dört satırın ikisi oraya adıyla işaret ediyor.

**Faz 6'nın kapanışında ölçülmüş hâli:** beş canlı workflow (Main 191 · Reminders 19 · Processed Purge 27
· Owner Actions 44 · Drill State Read 9), beşi de active, **beş sanitized export'un beşi de canlıya eşit**
· `run-d11` 8 vaka / 13 assert, 0 fail · `check-all` · `host-leak` · `secret-scan` · `hook-drift` ·
`deploy-separation` · `live-parity` · `exports-parity` · üç `tsc` · dashboard 10/10 · shared 32/32 hepsi 0.
