...
- ☐ **CP4 — Slot-filling (multi-turn booking):** for a `book` intent, collect `serviceId` / `date` / `time` across turns; `stage` transitions (`new` → `collecting` → `ready`). The availability check + the actual booking WRITE are Phase 3.
